...
`;
  ...
  const getTrueImageSize = ( img ) => {
    const clone = new Image();
    clone.src = img.currentSrc || img.src;
    return {
      width: clone.naturalWidth,
      height: clone.naturalHeight,
    };
  };
  ...
  const zoomImage = ( image, config ) => {
    ...
    const { width: naturalWidth, height: naturalHeight } = getTrueImageSize( image );
    ...
    if ( isPaddingNeeded ) {
      ...
      if ( !shouldExceed ) {
        const limitedScale = getScale(
            imageHeight,
            imageWidth,
            naturalHeight - config.padding,
            naturalWidth - config.padding,
        );
  ...
  const defaultConfig = {
    ...
    selector: `.wp-block-image img:not([data-image-zoom-disabled]), .wp-block-media-text img:not([data-image-zoom-disabled])`,
